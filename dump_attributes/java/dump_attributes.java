/* DumoAttributes.java, see http://ultramessaging.github.io/UMExamples/DumpAttributes/java/ */
...
public class DumpAttributes {
	...
	public static void main(String[] args) throws LBMException {
		...
		LBMConfigOption cfgOptArray[] = ctx.dumpAttributeList();
		for(int i = 0; i<cfgOptArray.length; i++)
		{
			System.out.println(cfgOptArray[i].getType() + " " + cfgOptArray[i].getOptionName() + " " + cfgOptArray[i].getValue());
		}
		System.out.println("Source #1 Attributes: ");
		cfgOptArray = src1.dumpAttributeList();
		for(int i = 0; i<cfgOptArray.length; i++)
		{
			System.out.println(cfgOptArray[i].getType() + " " + cfgOptArray[i].getOptionName() + " " + cfgOptArray[i].getValue());
		}
		System.out.println("Source #2 Attributes: ");
		cfgOptArray = src2.dumpAttributeList();
		for(int i = 0; i<cfgOptArray.length; i++)
		{
			System.out.println(cfgOptArray[i].getType() + " " + cfgOptArray[i].getOptionName() + " " + cfgOptArray[i].getValue());
		}
